...
def postprocess(input_dir, output_dir, min_total_length, log_file):
    """
    ...
    for filename in os.listdir(input_dir):
        ...
        with open_func(input_file_path, mode) as file:
        ...
        if total_length > min_total_length:
            ...
            output_filename = (
                os.path.splitext(os.path.splitext(filename)[0])[0] + ".fna"
                if is_gzipped
                else os.path.splitext(filename)[0] + ".fna"
            )
            output_file_path = os.path.join(output_dir, output_filename)

            with open_func(input_file_path, mode) as infile, open(
                output_file_path, "w"
            ) as outfile:
...
def main():
    ...
    postprocess(args.input_dirs, args.output_dir, args.minsize_bins, args.log)
